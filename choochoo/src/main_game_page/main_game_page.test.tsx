...
interface PlayerCardProps {
  ...
  jest.mock('./components/Profile/ProfileCard', () => ({
    __esModule: true,
    default: ({ username, trainCount, profilePic, main_player, active }: PlayerCardProps) => (
      <div data-testid={`player-card-${username}`}>
        ...
        <div data-testid={`active-${username}`}>{active ? 'Active' : 'Inactive'}</div>
      </div>
    )
  }));
  ...
  jest.mock('./components/PlayerActions/ActionBox', () => ({
    __esModule: true,
    default: ({ action, updateStatus, updateDrawDest, updateTrains, updateFaceUp, handleDrawPileClick }: ActionBoxProps) => {
      return (
        <div data-testid="action-box">
          <button data-testid="select-draw-train" onClick={() => updateStatus(1)}>Draw Train Cards</button>
          <button data-testid="select-play-train" onClick={() => updateStatus(2)}>Play Train Cards</button>
          <button data-testid="select-draw-dest" onClick={() => {
            updateStatus(3);
            updateDrawDest(true);
          }}>Draw Destination Card</button>
        </div>
      );
    }
  }));
  ...
  jest.mock('./components/Map', () => ({
    __esModule: true,
    default: ({ routes, hoveredRoute, setHoveredRoute, onRouteClaim }: MapProps) => {
  ...
  describe('MainGamePage', () => {
    ...
    test('cannot claim a route if not in play train cards mode', () => {
      ...
      expect(screen.getByTestId(`route-claimer-${routeIndex}`)).toHaveTextContent('unclaimed');
    });
  
     // FAILED FAILED FAILED FAILED FAILED
    test('drawing train cards works and shows notification', () => {
      render(<MainGamePage />);
      
      // action to draw train cards
      fireEvent.click(screen.getByTestId('select-draw-train'));
      
      // create a custom event to trigger drawing a card
      const drawCardEvent = new Event('drawCard');
      window.dispatchEvent(drawCardEvent);
      
      // does a notification appear?
      expect(screen.getByText(/You drew a .* train card!/)).toBeInTheDocument();
      
      // does notification disappear after 3 seconds?
      act(() => {
        jest.advanceTimersByTime(3000);
      });
      
      expect(screen.queryByText(/You drew a .* train card!/)).not.toBeInTheDocument();
    });
    ...
    test('drawing destination cards shows the DrawDestinationCard component', () => {
      ...
      expect(screen.queryByTestId('draw-destination-card')).not.toBeInTheDocument();
    ...
    test('claiming a route with insufficient train cards fails', () => {
      ...
      expect(screen.getByTestId('route-claimer-9')).toHaveTextContent('unclaimed');
    });
    ...
    test('claiming a route with wild cards works', () => {
      ...
      expect(screen.getByTestId('route-claimer-18')).toHaveTextContent('noah-rama');
